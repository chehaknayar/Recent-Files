...
def listfreq(listt,doc): #finds freq of words  of a list in a doc and returns sum of all the feqencies
	print 'list sent to function'
	print listt
	print 'sentence sent to function'
	doc=doc.lower()
	doc=doc.split()
	for z in xrange(len(doc)):		#putting all words in list of the form [' word1 ',' word2 ']
					doc[z]=" "+doc[z]+" "	
	print doc
	summ=0
	count=0
...
def idf(listt, sent):
	...
	return 1+ math.log(len(sent)/float(num_sentences_containing(listt,sent)))
...
#																																					   !
				...
				cur.execute('INSERT INTO OriginalAnswers(Words , Q_Num , Ans_Num , POS , FreqTf , Dft, NormalisedTf, IDft, TfIdf, Similar, cosSim) VALUES (?,?,?,?,?,?,?,?,?,?,?);',(w,1,i,a[x][1],0,c,tff,idff,t,similarr,cs))
